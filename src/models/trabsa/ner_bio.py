...
the tweet text keeps its original casing and punctuation for the encoder.
...
def _match_key(word: str) -> str:
    ...
    only — TRABSA's cleaned tweet text keeps casing/punctuation (the encoder
    wants it), but that means 'Messi!' and 'Messi' must still be recognised
    as the same word when matching entity names against the tweet."""
