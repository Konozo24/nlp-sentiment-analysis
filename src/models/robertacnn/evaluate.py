"""Evaluate RobertaCNN on the test split — the same tweets TRABSA was scored on.
    ...
    for task in all_tasks:
        ...
        lines.append(
            classification_report(
                true[task], pred[task], labels=range(len(labels[task])),
                target_names=labels[task], digits=4, zero_division=0,
            )
        )
        if task == "ner":
